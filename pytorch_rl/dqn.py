...
from rl.replay import Transition, ReplayMemory
...
def select_action(state):
    ...
    if sample > eps_threshold:
        return model(
            Variable(state.unsqueeze(0), volatile=True).type(FloatTensor)).data.max(1)[1].view(1, 1)
    else:
...
#         memory.push(state, np.array([[action]]), next_state, reward)
#
...
for i_episode in range(num_episodes):
    ...
    if i_episode % 10 == 0:
        reporter.report(i_episode, {'rewards': episode_rewards})
