# Adapted from fairseq-py (https://github.com/facebookresearch/fairseq-py)
...
class progress_bar(object):
    ...
    def _format_stats(self, stats):
        ...
        for key in postfix.keys():
            ...
            elif isinstance(postfix[key], AverageValueMeter):
                postfix[key] = '{:.2f} ({:.2f})'.format(
                    postfix[key].val, postfix[key].mean)
...
class json_progress_bar(progress_bar):
    ...
    def _format_stats(self, stats, epoch=None, update=None):
        ...
        for key in stats.keys():
            # Meter: display both current and average value
            if isinstance(stats[key], AverageValueMeter):
                postfix[key] = stats[key].val
                postfix[key + '_avg'] = stats[key].mean
            else:
